...
public class VulkanModVersionHelper {
    ...
    private static Boolean memoryPoolingSupported = null;
    private static Boolean bufferClassAvailable = null;
    ...
    /**
     * Check if memory pooling features are supported (requires VulkanMod 0.5.5+)
     */
    public static boolean isMemoryPoolingSupported() {
        if (memoryPoolingSupported == null) {
            // First check version requirement
            boolean versionSupported = isVulkanModVersionAtLeast("0.5.5");

            // Then check if the Buffer class exists
            boolean bufferAvailable = isBufferClassAvailable();

            memoryPoolingSupported = versionSupported && bufferAvailable;

            VulkanModExtra.LOGGER.info("Memory pooling support: version={}, buffer_class={}, supported={}",
                versionSupported, bufferAvailable, memoryPoolingSupported);
        }
        return memoryPoolingSupported;
    }

    /**
     * Check if the VulkanMod Buffer class is available
     */
    public static boolean isBufferClassAvailable() {
        if (bufferClassAvailable == null) {
            try {
                Class.forName("net.vulkanmod.vulkan.memory.buffer.Buffer");
                bufferClassAvailable = true;
                VulkanModExtra.LOGGER.debug("VulkanMod Buffer class found");
            } catch (ClassNotFoundException e) {
                bufferClassAvailable = false;
                VulkanModExtra.LOGGER.debug("VulkanMod Buffer class not found: {}", e.getMessage());
            }
        }
        return bufferClassAvailable;
    }
    ...
    public static String getCompatibilitySummary() {
        return String.format("VulkanMod %s - Memory Pooling: %s",
            getVulkanModVersion(),
            isMemoryPoolingSupported() ? "Supported" : "Not Supported");
    }
    ...
    public static void reset() {
        ...
        memoryPoolingSupported = null;
        bufferClassAvailable = null;
    }
}